...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'Redline' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor("Redline Roleplay Server Status", "https://cdn.discordapp.com/attachments/726534671097135208/726559880579776563/logodc.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Redline Roleplay' : '⚫ Redline Roleplay')
    .setTimestamp(new Date())
    .addField('\n\u200b\nA csatlakozáshoz: *FiveM --> F8 --> connect play.redlinehungary.hu*\n\u200b\n', true)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Szerver státusza',`${STATUS}\n\u200b\n`);
  ...
  const offline = function() {
    ...
    if (LAST_COUNT !== null) log(LOG_LEVELS.INFO,`A szerver offline ${URL_SERVER} (${URL_PLAYERS} ${URL_INFO})`);
    let embed = UpdateEmbed()
    .setColor(0xff0000)
    .addField('Server Status',':x: Offline',true)
    .addField('Várakozási sor','?',true)
    .addField('Online játékosok','?\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('Sor',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Játkosok',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
      ...
      if (message.member) {
        if (message.member.hasPermission('ADMINISTRATOR')) {
          if (message.content.startsWith('+status ')) {
            ...
            .setColor(0x2894C2)
            .setTitle('Frissített állapotüzenet')
            .setTimestamp(new Date());
            if (status === 'clear') {
              STATUS = undefined;
              embed.setDescription('Állapotüzenet törölve!');
            } else {
              STATUS = status;
        ...
        if (message.channel.id === SUGGESTION_CHANNEL) {
          ...
          message.channel.send(embed).then((message) => {
            ...
            sent.react('👍').then(() => {
              sent.react('👎').then(() => {
                log(LOG_LEVELS.SPAM,'Befejezett ötlet');
              }).catch(console.error);
            }).catch(console.error);
          }).catch(console.error);
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setAuthor(message.member.nickname ? message.member.nickname : message.author.tag,message.author.displayAvatarURL)
          .setColor(0x2894C2)
          .setTitle('Jelentés Matteo Design')
          .setDescription('A jelentésedet sikeresen rögzítettük!')
          .setTimestamp(new Date());
  ...
  bot.login(BOT_TOKEN).then(null).catch(() => {
    log(LOG_LEVELS.ERROR,'Nem sikerült bejelentkezni Ellenőrizze a bejelentkezési tokent');
    console.error(e);
    process.exit(1);
  });
